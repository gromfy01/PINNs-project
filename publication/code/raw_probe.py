...
def parse_job_name(name: str) -> Dict[str, float]:
    ...
    ВНИМАНИЕ к смыслу `rd`. Токен `rd` — обжатие ПО ДИАМЕТРУ, а не по площади.
    Измерено по геометрии сетки (R₀ = 18 мм из имени волоки `DIE_..._D0_36`,
    радиус протянутой части — 99.5-й перцентиль по узлам первых 30 % длины):

        rd/1e4  R прот., мм   1 − R/R₀   1 − (R/R₀)²
        0.0150     17.710      0.0161      0.0320
        0.0500     16.996      0.0558      0.1084
        0.1000     16.087      0.1063      0.2013
        0.1500     15.159      0.1579      0.2908
        0.2000     14.087      0.2174      0.3876
        0.2500     13.334      0.2592      0.4512

    Средняя |rd − линейное обжатие| = 0.007 против |rd − площадное| = 0.105,
    то есть в 15 раз ближе к линейному. Поэтому `Q` ниже — линейная величина,
    а истинное обжатие по площади равно 1 − (1 − Q)² и примерно вдвое больше;
    оно возвращается отдельным ключом `Q_area`.

    На обученные модели это НЕ влияет: Q входит в сеть как обусловливающий
    признак, а Q → Q_area — строго монотонное преобразование, которое
    поглощается первым слоем. Влияет на ФОРМУЛИРОВКИ: диапазон надо
    приводить как «обжатие по диаметру 1.5–25 %, по площади 3.0–45 %»,
    иначе сравнение с литературой, где принято обжатие по площади,
    завышает степень деформации вдвое.
    """
    s = name.split("_")
    out: Dict[str, float] = {}
    for i, t in enumerate(s):
        if t in ("red", "rd"):
            out["Q"] = float(s[i + 1]) / 10000          # обжатие по ДИАМЕТРУ
            out["Q_area"] = 1.0 - (1.0 - out["Q"]) ** 2  # по площади
        elif t == "cal":
