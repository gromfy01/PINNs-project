...
EQUILIBRIUM_R2_STATUS = (
    "редуцированная форма НЕ оправдана: отброшенный ∂σ_zz/∂z составляет "
    "68 % (медиана) от удержанных членов, невязка меняется на 32 %; "
    "выражение ∂τ_rz/∂r + τ_rz/r нельзя называть осевым равновесием")
...
def shear_balance_residual(sigma_profile: np.ndarray, r_grid: np.ndarray,
                           exclude_boundary: bool = True) -> np.ndarray:
    """
    ∂τ_rz/∂r + τ_rz/r. Намеренно НЕ называется «осевым равновесием»:
    без ∂σ_zz/∂z это не оно (EQUILIBRIUM_R2_STATUS). Оставлено как
    регуляризатор гладкости сдвига и как метрика сопоставимости с
    отклонённой версией.
    """
