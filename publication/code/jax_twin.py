...
Смысл конструкции. Чтобы вопрос «важен ли бэкенд» имел ответ, сравнивать надо
две реализации, у которых совпадает ВСЁ, кроме движка автодифференцирования:
архитектура, начальные веса, порядок батчей, оптимизатор, число эпох, критерий
ранней остановки. Здесь начальные веса генерируются одним и тем же numpy-RNG,
что и в torch-версии, и порядок перемешивания батчей воспроизводится тем же
алгоритмом. Тогда остаточная разница — это численные различия движков и
реализации AdamW, и ничего больше.
...
же PyTorch за другим API (ERRATA E-19). Проверка `check_gradient_agreement()`
показывает, что производные torch и JAX на одних и тех же весах совпадают до
машинной точности — то есть разница в метриках между «бэкендами» не может
быть отнесена на автодифференцирование в принципе.
"""
...
def init_params(cfg: Config, n_in: int = 6, n_out: int = 4) -> List[Tuple[np.ndarray, np.ndarray]]:
    """
    Glorot normal тем же генератором, что и в torch-версии: torch.manual_seed(seed)
    здесь не поможет, поэтому обе версии инициализируются ЯВНО из numpy-RNG с
    одним сидом — это и делает сравнение честным.
    """
