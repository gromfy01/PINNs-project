...
In this study, no direct experimental validation of the finite element model was carried out (i.e. no measurement of the drag force or residual stresses using X-ray diffraction). The modelling approach employed in this study aligns with established practices in the numerical analysis of wire drawing. This approach encompasses the utilisation of finite elements, a contact formulation, and a material description based on the Johnson–Cook model. This practice, as demonstrated in previous independent studies \citep{shin2026accurate,he2003residual,overstam2006influence,selbmann2024residual}, is in good agreement with experimental measurements, which serves as indirect confirmation of the model's reliability. Nevertheless, it should be noted that such an agreement does not constitute direct validation of the specific dataset obtained here. Consequently, it is imperative that the resulting residual stress values are interpreted with this limitation in mind.
...
\begin{figure*}[h!]
    \centering
    \includegraphics[width=\linewidth]{pictures/fig_contour_raw.png}
    \caption{Residual stress components from one finite-element run over the wire cross-section; the dashed lines mark the 25--75\,\% axial window retained for training.}
    \label{fig:contour-raw}
\end{figure*}
...
is not reported separately \citep{lu2021}.
...
\begin{figure*}[t]
    ...
    \caption{Distribution of residual stresses obtained from the FEM simulation at $Q = 0.10$ (10\,\% reduction in diameter, 20.1\,\% in area), $k = 0.5$, $\alpha = 12^\circ$, $\mu = 0.05$, $v = 10$~m/min. Components $\sigma_{rr}$, $\sigma_{\theta\theta}$, $\sigma_{zz}$ and $\tau_{rz}$ over the full axial extent of the wire; $r$ is normalised by the billet radius $R = 18$~mm, $z$ by the model length $L = 139.5$~mm. Dashed lines mark the axial window 25--75\,\% of $L$ from which the training data are taken.}
...
In admissibility they are not (Fig.~\ref{fig:residual-maps}). This figure replaces Figs.~11--12 of the previous version, whose boundary metric was evaluated at $r = 1.5R$, outside the material, on fields with wrong component labels. % src: ERRATA.md E-01 (R_EXTRAP_FREE_SURFACE = 1.5; condition imposed at 1.5R, measured at r = 1), E-18 (component labels); MANUSCRIPT.md §6.2 % EDIT: sentence added (critic)
