...
% same window would give 59.2 / 65.5 / 77.5 / 62.4 MPa, the upper edge of the estimator band.
...
Both relations measure the friction through the factor $m$, the ratio of the interface shear
stress to the shear yield stress, whereas the simulations are driven by a Coulomb coefficient
$\mu$. The conversion $m = \sqrt3\,\mu$ holds only where the normal pressure on the contact equals
the flow stress; it is an assumption about the contact, not a change of units. Vega and co-workers
\citep{vega2009temperature,vega2009investigation,haddi2011analysis} write the same upper bound with
$\mu$ entering directly, so that no conversion is needed, and treat the land as a bearing along
which the axial stress grows under Coulomb friction:
\begin{equation}
...
Eq.~\eqref{eq:avitzur-vega} is the linearisation; the exact form differs from it by at most
4.1\,\% at the present parameters. Equation~\eqref{eq:avitzur-vega} therefore carries an explicit
...
lubricant and make it a function of the interface temperature,
\begin{equation}
...
bearing assumption, not an independent prediction. Reduction 1.6\,\%, land ratio unity, drawing
speed 40~m/min.}
...
\end{tabular}
\end{table*}
% src: publication/reexport/verification_table.py --latex; FEM row from drawing_force.py
% (time mean of |RF2| over die travel 20-100 mm, divided by the measured outlet section);
% sigma_f 302.4/301.1/305.3/299.9 MPa from flow_stress.py; the finite-element slope is 323 MPa
% per unit mu over the same two points.

Both upper-bound columns lie above the computed drawing stress at every point, as an upper bound
must, but the margins are not small and they order themselves by how each relation treats the land:
36.2\,\% on average in the Coulomb form and 55.2\,\% in the friction-factor form. Siebel is the
closest, at 17.3\,\%, and it is the only one of the three with no land term at all; it is also the
only one that falls below the computed value, which it does at two of the four points. At a land
ratio of unity the land is 89--93\,\% of the contact area, so this is where the comparison is
decided.
% src: verification_table.py (contact areas: land 1970 mm^2 against cone 156 mm^2 at 12 deg,
% 236 mm^2 at 8 deg)

The exported contact reaction locates the error. Equation~\eqref{eq:avitzur-vega} implies a
pressure on the land of 240--251~MPa through its yield closure $p=\sigma_f-\sigma_z$, whereas the
mean contact pressure in the model is 122~MPa, a factor of 1.97 to 2.06 lower; the friction-factor
form is further out still, since $m=\sqrt3\,\mu$ places the land at the shear yield stress
outright. The sensitivity to friction confirms the diagnosis independently of the magnitudes: from
...
the mean deviation to 17.8\,\% and the slope to 303~MPa per unit $\mu$, within 6\,\% of the model.
Material leaving a 1.6\,\% reduction is barely above yield, and the land carries it as a sliding
contact held by elastic recovery rather than as a plastically loaded bearing; that, and not the
calibration of the flow stress, is what both bearing models get wrong. The Coulomb branch itself
remains admissible throughout: $\mu p$ reaches 25~MPa at most against a shear yield stress of
173--176~MPa, so nowhere does the contact approach sticking.
% src: verification_table.py (assumed vs measured bearing pressure, sticking limit)
...
row. The offset lies in the redundant-shear term, which carries 30 to 65\,\% of the predicted
drawing stress here against 8 to 13\,\% for the ideal work. The inhomogeneity parameter is 17.1 at
...
coefficient at the initial 20~$^\circ$C would multiply it by 2.4 to 3.1 at the pass-mean
temperature, or by 4.6 to 6.1 at the reference temperature of the original calibration, in either
...
supply.
% src: verification_table.py (pass-mean temperature 26.6-30.6 C; multipliers at T_0 = 20 and 15.6 C)
...
1.6 but the flow stress by 1.6\,\% and the estimate below by the same, because at
...
iterations. The friction on the land is deliberately left out of this balance: its heat is released
in a thin surface layer and is shared with the die, so it does not raise the mean temperature of
the section. Carrying it into the bulk balance instead would raise the estimated rise by 19 to
84\,\% and break the agreement with the measured values of Table~\ref{tab:deltaT}. For the four runs the fixed
point returns a flow stress of 330--338~MPa, consistent with the value derived from the
constitutive equation (Section~\ref{sec:prep-data}).
...
stress, $\sigma_d^{\mathrm{def}}$, which excludes the friction on the land for the reason given in
the text. Nothing is taken from the finite-element model; $\varepsilon_{\mathrm{hom}}=0.032$ and
$R_f=17.71$~mm throughout.}
\label{tab:closed}
...
assumed thermal boundary condition. Temperatures are stated in degrees Celsius throughout, the
...
temperature history. Every run starts at 20~$^\circ$C. Reduction 1.6\,\%, land ratio unity.}
\label{tab:deltaT}
\begin{tabular}{@{}llrrr@{}}
\toprule
$\alpha$ & $\mu$ & $\Delta T$ analytical, $^\circ$C & $\Delta T$ measured, $^\circ$C & $T$ pass mean, $^\circ$C \\
\midrule
$12^\circ$ & 0.025 & 16.3 & 15.0 & 28.2 \\
$12^\circ$ & 0.050 & 16.6 & 16.9 & 28.3 \\
$12^\circ$ & 0.100 & 17.3 & 21.3 & 28.6 \\
$8^\circ$  & 0.100 & 13.1 & 15.8 & 26.6 \\
\bottomrule
\end{tabular}
\end{table}
% src: publication/reexport/analytic_sigma_f.py --Q 0.0161 (analytical, pass mean T = T0 + dT/2);
% publication/reexport/drawing_force.py (measured: nodal peaks over the history, averaged over the
% radius with the area weight; the peak surface rise is 45.7-74.8 C, see the text).
...
pressure is 122~MPa and varies by less than 1~MPa with either the semi-angle or the friction, as it
must when a cylindrical land is 89--93\,\% of the contact area. And the flow stress recovered from
the heat alone, over the core of the section where friction contributes none, is 306--320~MPa, in
agreement with the value derived from the constitutive equation.
...
range of 1 to 3 for which the upper-bound solutions were derived; the ideal work accounts for 8 to
13\,\% of the predicted drawing stress and the total work exceeds it by a factor of 7.6 to 12.5.
