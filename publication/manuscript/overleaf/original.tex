...
%% Класс подтягивает array и booktabs, но не tabularx, а Таблица 1 набрана им.
\usepackage{tabularx}

%% Подвал первой страницы. Класс ставит в одну строку название журнала слева,
%% копирайт по центру и номер страницы справа. При \@JourName = "Manufacturing
%% Science and Engineering" левый элемент занимает 290 pt из 492, и копирайт
%% налезает на него. Ни один кегль не спасает: сумма трёх элементов шире строки.
%% Копирайт опускается на отдельную строку под подвалом.
\makeatletter
\fancypagestyle{title}{%
    ...
    \fancyfoot[CO]{\raisebox{-1.35\baselineskip}[0pt][0pt]{%
        \normalsize\bfseries\sffamily Copyright \textcopyright\ \@PaperYear\ by ASME}}%
}
\makeatother
...
\hypersetup{%
	...
	pdfkeywords={physics-informed neural networks, wire drawing, residual stresses, variational PINN, finite element simulation, surrogate modelling},
...
\SetAuthorBlock{Dmitriy Demin\CorrespondingAuthor}{
HSE University,\\
...
\SetAuthorBlock{Georgii Romanenko}{HSE University,\\
   11 Pokrovsky Boulevard,\\
   Moscow, Russia \\
...
\keywords{physics-informed neural networks, wire drawing, residual stresses, variational PINN, finite element simulation, surrogate modelling}
...
%% Шаблон ставил сюда \date{Version \versionno, \today}, то есть версию КЛАССА
%% (1.18) и дату сборки; в сноске первой страницы это выглядело как версия статьи.
\date{}  
...
Material behaviour is described by the Johnson--Cook model, with the constants of the
input deck: $A = 213$~MPa, $B = 53$~MPa, $n = 0.345$, $m = 0.81$, melting temperature
$1386^\circ$C and transition temperature $20^\circ$C for the thermal term, and
$C = 0.055$ with a reference strain rate of $0.004$~s$^{-1}$ for the rate term. Elastic
constants are $E = 190$~GPa and $\nu = 0.29$; the density is 7870~kg\,m$^{-3}$, the
specific heat 470~J\,kg$^{-1}$K$^{-1}$ and the inelastic heat fraction 0.9. The analysis
is a coupled temperature--displacement one and is carried out in degrees Celsius, with an
initial temperature of $20^\circ$C throughout. The flow stress therefore follows the constitutive
equation
\begin{equation}
\sigma_f = \bigl(A + B\varepsilon^{\,n}\bigr)
           \left(1 + C\ln\frac{\dot\varepsilon}{\dot\varepsilon_0}\right)
           \bigl(1 - \theta^{\,m}\bigr), \qquad
\theta = \frac{T - T_{\mathrm{tr}}}{T_{\mathrm{melt}} - T_{\mathrm{tr}}},
\label{eq:johnson-cook}
\end{equation}
in which $T_{\mathrm{tr}} = 20^\circ$C is the transition temperature of the thermal term and
$\dot\varepsilon_0 = 0.004$~s$^{-1}$ the reference strain rate. It is defined only at stated
values of strain, strain rate and temperature, which is why no single number describes the pass.
% src: .inp deck (*Plastic, hardening=JOHNSON COOK; *Rate Dependent, type=JOHNSON COOK;
% *Elastic; *Density; *Specific Heat; *Inelastic Heat Fraction), supplied 12-13.09.2026.

All three arguments of the constitutive equation vary strongly across the section, so no
single yield stress describes the pass. Over the four runs for which the strain, strain-rate
and temperature histories were exported, the equivalent plastic strain runs from 0.034 on the
axis to 0.46--0.58 at the surface, the strain rate from 3 to 99~s$^{-1}$, and the temperature
from the initial $20^\circ$C to 66--95$^\circ$C at the surface. Because the thermal exponent
is below unity, the homologous temperature enters with disproportionate weight: a rise of
75~K gives a homologous temperature of 0.055 but a softening of 9.5\,\%.
% src: publication/reexport/flow_stress.py on publication/reexport/history/ (four unique runs)

Where a single representative flow stress is needed -- in the comparison with the analytical
solutions of Section~\ref{sec:res_fem_verification}, which are energy methods -- it is formed
by averaging the constitutive equation \eqref{eq:johnson-cook} along the deformation path of
each material point, weighted by plastic work, and then over the section weighted by radius:
$\sigma_f = \int\sigma(\varepsilon,\dot{\varepsilon},T)\,\mathrm{d}\varepsilon_p /
\int\mathrm{d}\varepsilon_p$. Evaluated on the exported histories this gives
$\sigma_f = 300$--$305$~MPa, with a node-to-node spread of 260--324~MPa across the section.
% src: publication/reexport/flow_stress.py
% REV: the earlier text recovered an effective yield stress of 320 MPa from the stress field
% and used it as if it were a material constant. The reviewer's objection is accepted: the
% flow stress follows from the constitutive equation and is defined only at stated values of
% strain, strain rate and temperature. The value is now derived rather than postulated.
The same constitutive description was used in all simulations, so the resulting dataset reflects pure variation of geometry and process conditions. Spatial coordinates of mesh nodes and the four cylindrical components of the residual stress tensor were exported from the Abaqus Output Database (\texttt{.odb}) files through scripts written against the Abaqus CAE Python API. Only the nodes lying within 25--75\% of the wire cross-section height were retained for surrogate-model training, because this region corresponds to the steady-state deformation regime and is most representative of the residual stress field of interest. Each retained set was downsampled to 20 radial points so that all simulations share the same spatial grid.
...
% NOTE:, K27: the paragraph no longer opens by declaring the absence of experimental validation; the approach is stated positively, the scope of the conclusions is named once, and the reader is sent to the verification.
...
% NOTE: the figure pictures/fig_contour_raw.png removed. It duplicated
...
% NOTE: the sentence was recast; the shares and the losses are unchanged.
...
The shear component is thus intrinsically unpredictable from a one-dimensional representation, a property of the data rather than of any model (Fig.~\ref{fig:fem-fields}: inside the window the mean $|\tau_{rz}|$ of the shown run is 1.06~MPa against 11.0~MPa outside). % src: QA_DATA_AND_Z.md §5 (job 1020_2a_24_rd_1000_cal_50_v_10_fric_050); fig:fem-fields = figures/paper/fig2_fem_fields.png (same job: Q = 0.10, k = 0.5, α = 12°, μ = 0.05, v = 10; caption in manuscript/captions.md Fig. 2) % EDIT: fig:contour-raw → fig:fem-fields, the paper version of the same run; TODO: the figure environment itself is not in any manuscript file yet

% EDIT: coordinate-normalisation note moved from the figure captions into the text (captions.md "Что переезжает в текст"; review's instruction).
% NOTE: the numerical value of the billet radius is not repeated in the text; it is retained in the caption of Fig.~\ref{fig:fem-fields}. % NOTE: with the value gone, $R$ was left undefined here and clashed with $R$ = surface radius used elsewhere in this subsection; the sentence now says which radius the axis label of Fig.~\ref{fig:fem-fields} denotes.
...
\begin{table*}[htbp]
\centering\small
\caption[Variance along $z$ and the effect of collapsing the field]{Share of the variance along $z$ inside the axial window and effect of collapsing the field along $z$ by the median (mean absolute value, MPa).} % src: QA_DATA_AND_Z.md §4; TODO: state the population (all 2443 runs or the filtered set) — not stated in the source
\label{tab:zshare}
\begin{tabular}{@{}lccc@{}}
\toprule
...
% NOTE: "nodal noise" replaced by an explicit explanation of the numerical origin of the effect; the figures are unchanged.
...
\paragraph{Filtering.}
% NOTE: an explanatory opening sentence was added before the list of the filters.
...
% NOTE: the single run with a mistyped friction label is no longer mentioned in the running text; it is retained as a row of Table~\ref{tab:filtering}. % src: ERRATA.md E-03
...
% EDIT: sentence "Removing the last 5.0 % of cases changed the results materially; the comparison with the uncleaned data is given in Section ..." deleted — §3.2 contains no comparison with the uncleaned data and v1 figures are not used

\paragraph{Group-aware splits.}
% NOTE: an introductory sentence was placed before the description of the partitions.
...
% NOTE: the number of points per computation was removed; the uniqueness of the configurations produced by the design and the fact that the partitioning is performed at the level of computations are stated instead.
...
% NOTE: the description of the controls and of the joint-range partition was recast in more formal terms; the content is unchanged.
...
% NOTE: the term "corner split" was replaced by a description in terms of a joint departure from the ranges of two factors; $\alpha$ is named the die semi-angle, i.e.\ the half-angle of the deformation zone.
...
\begin{table*}[htbp]
\centering\small
\caption[The 13 case-level splits of the 1756 cases]{The 13 case-level splits of the 1756 cases. $n_\text{train}$ is the training pool, including the early-stopping subset.} % src: results/runs_v2.csv (n_train, n_test, stage main; 13 distinct splits)
\label{tab:splits}
\begin{tabular}{@{}lllrr@{}}
\toprule
...
\texttt{extrap\_joint:corner}& $\alpha = 20^\circ \wedge Q \ge 0.15$ & gap in the joint $(\alpha, Q)$ range & 1642 & 114 \\ % NOTE: "joint-space gap" named after the two factors involved % src: results/runs_v2.csv
...
\begin{figure*}[h!]
    \centering
    \includegraphics[width=\linewidth]{pictures/mlp_schematic.pdf}
    \caption[Training of the data-driven MLP surrogate]{Training of the data-driven MLP surrogate. The architecture and every
    hyperparameter are fixed before any held-out region is evaluated; the loop shown
    is the training loop of a single run, and no hyperparameter search surrounds it.}
    \label{fig:architecture}
\end{figure*}

\begin{figure*}[h!]
    \centering
    \includegraphics[width=\linewidth]{pictures/pinn_schematic.pdf}
    \caption[Training of the physics-informed surrogate]{Training of the physics-informed surrogate: shared trunk, equilibrium
    residual and traction-free boundary term in the loss. The loss weights are fixed
    a priori; the only quantity read from the data is the gradient-norm calibration
    $g$ of Eq.~\eqref{eq:calib}, set once on the first mini-batch.}
    \label{fig:architecture-pinn}
\end{figure*}

% >>> inlined sections/sec23_body.tex
\label{sec:models}
% NOTE: introductory construction added before the description of the models.
% NOTE:/K13: the conditions are equal within a track, not across tracks — the epoch limit, mini-batch size, patience and number of seeds differ between the main and the extended track (see ``Training protocol'').
...
% NOTE: ``training budget'' spelled out (epoch limit, early-stopping criterion with its patience, mini-batch size and batch order, seeds); the values differ between the tracks and are given in ``Training protocol''.
...
Figs.~\ref{fig:architecture} and~\ref{fig:architecture-pinn}). In the main track the input is
...
% NOTE: the paragraph opens with the equations of equilibrium as the basis of the physics-informed families, notes that the quantities entering them depend on $r$ and $z$, and states that in preparing the models those relations undergo a number of changes.
% TODO: still needed are the two references on the PINN formulation for the equilibrium equations of a continuum to be cited here; the keys are not in the bibliography of publication/ and must be supplied by him.
...
\begin{table*}[t]
% NOTE: ``budget'' in the caption spelled out (epoch limit, early-stopping rule, mini-batch size); the scope is limited to one track, since the extended track uses its own seeds and splits.
\caption[Model families and the composition of their loss]{Model families. Within each track all families share the network, the
...
% NOTE: introductory construction added before the protocol.
...
early-stopping subset of 15\,\% of the training cases with patience 40 (30), and % EDIT: "validation subset" → "early-stopping subset"
...
% NOTE: the statement on the fixed architecture and hyperparameters moved from here to the closing paragraph of the subsection.
...
the seed governs initialisation, batch order and the internal early-stopping subset, % EDIT: "validation subset" → "early-stopping subset"
...
% EDIT: E-12 caveat added. src: run_matrix.py l.188/202 (from learning_curves import nested_subsamples); learning_curves.py (stratified_nested_subsamples exists, unused by the launcher); ERRATA.md E-12 (122 cases at v = 5)
...
% EDIT: the "Evaluation splits" table (a second \label{tab:splits}) deleted — the full 13-split table with n_train/n_test is Table~\ref{tab:splits} in Section~\ref{sec:prep-data}; its caption sentence on the 13.2 % friction-label duplicates was moved to §2.2 "Factorial design".
...
\paragraph{Backends.}
% NOTE: "training budget" replaced by an explicit statement of what the twin shares (the epoch limit and the early-stopping rule).
...
% NOTE: stated as a property of the library rather than as a correction of an earlier text.
...
question at issue.
% NOTE: the point is kept, the reference to an earlier version of the text removed.
...
analytical solutions for the drawing stress, those of Siebel and of Avitzur and the modification
of the latter due to Vega and co-workers \citep{vega2009temperature}, and by internal consistency
tests of the exported fields.
...
The drawing stress is then
\begin{equation}
\sigma_d = \frac{F}{S}, \qquad S = \pi R_f^2,
\label{eq:sigma-d}
\end{equation}
and with the area actually reached in the model, $S = 985$~mm$^2$ at an outlet radius of
17.71~mm, this gives 54.1, 62.0 and 78.3~MPa at $\alpha = 12^\circ$ for $\mu = 0.025$, 0.05 and
0.1, and 62.3~MPa at $\alpha = 8^\circ$.
...
\begin{table*}[htbp]
\centering\small
\caption[Drawing stress: finite element against the three closed-form relations]{Drawing stress (MPa) for the four runs: the finite-element value against the Siebel
relation, Eq.~\eqref{eq:siebel}, the Avitzur upper bound in its friction-factor form,
Eq.~\eqref{eq:avitzur} with $m=\sqrt3\,\mu$, and the modified Avitzur relation in the Coulomb
form of Eq.~\eqref{eq:avitzur-vega}. All three use the flow stress derived from the constitutive
equation, 300--305~MPa, and no fitted parameter. Reduction $Q$ is nominal, in diameter; the
achieved outlet radius is 17.71~mm in every run. The last two rows are the mean absolute
deviation from the finite-element value and the secant slope $\mathrm{d}\sigma_d/\mathrm{d}\mu$
between the $\mu=0.025$ and $\mu=0.1$ rows at $\alpha=12^\circ$, for which the finite-element
value is 323~MPa.}
\label{tab:drawstress}
\begin{tabular}{@{}ccccc rrr r@{}}
\toprule
\multicolumn{5}{c}{Parameters} & \multicolumn{3}{c}{Analytical} & Simulation \\
\cmidrule(r){1-5}\cmidrule(lr){6-8}\cmidrule(l){9-9}
$Q$ & $\alpha$ & $v$, m/min & $k$ & $\mu$ & Siebel & Avitzur & Avitzur\_mod & FEM \\
\midrule
0.015 & $12^\circ$ & 40 & 1.0 & 0.025 & 53.2 & 75.1 & 71.5 & 54.1 \\
0.015 & $12^\circ$ & 40 & 1.0 & 0.050 & 54.2 & 91.1 & 82.8 & 62.0 \\
0.015 & $12^\circ$ & 40 & 1.0 & 0.100 & 57.4 & 125.4 & 104.4 & 78.3 \\
0.015 & $8^\circ$  & 40 & 1.0 & 0.100 & 44.7 & 109.0 & 90.8 & 62.3 \\
\midrule
\multicolumn{5}{@{}l}{mean absolute deviation} & 17.3\,\% & 55.2\,\% & 36.2\,\% & --- \\
\multicolumn{5}{@{}l}{$\mathrm{d}\sigma_d/\mathrm{d}\mu$ at $\alpha=12^\circ$, MPa} & 57 & 670 & 439 & 323 \\
\bottomrule
\end{tabular}
\end{table*}
...
Substituting the computed contact pressure into the bearing term of Eq.~\eqref{eq:avitzur-vega}
and changing nothing else gives 66.1, 73.1, 88.8 and 73.3~MPa for the four runs: a mean deviation
of 17.8\,\% instead of 36.2\,\%, and a slope of 303~MPa per unit $\mu$, 6.1\,\% below the
...
\begin{table*}[htbp]
\centering\small
\caption[Intermediate quantities of the closed self-consistent estimate]{Intermediate quantities of the closed self-consistent estimate,
...
\begin{table}[htbp]
\centering\small
\caption[Mean temperature rise and pass-mean temperature]{Mean temperature rise during the pass and the resulting pass-mean temperature: the
...
\begin{table*}[t]
\caption[Cost of holding out a whole factor level]{Cost of holding out a whole factor level, averaged over the three families: macro-RMSE on the held-out region, on the matched control of the same size, their ratio, $R^2_{\mathrm{n}}$ (global denominator) on the held-out region, and the constant predictor (MPa).} % EDIT: denominator named
\label{tab:axis}
\centering\small
...
\begin{table*}[t]
\caption[Learning curves against the fraction of the training pool]{Learning curves: macro-RMSE (MPa) versus the fraction of the training pool, three seeds. $\Delta$ is PINN minus MLP with the seed-paired $p$-value. Slope of $\Delta$ on $\log(\text{fraction})$: $+0.20$~MPa ($p=0.21$) on \texttt{random}, $-0.35$~MPa ($p=0.070$) on \texttt{extrap:alpha\_max}.}
\label{tab:curves}
\centering\small
...
% NOTE: the learning curves are referred to the table that carries their values; no figure of them is set in the manuscript.
% TODO: should a figure of the learning curves be wanted, it is produced by publication/experiments/figures.py; at present the numbers are given by Table~\ref{tab:curves}.
...
% NOTE: stated as a result of the present comparison rather than as a withdrawal. Where the results are equivalent the cheaper implementation should be used. Here PyTorch completes a full training run 1.65~times faster ($313\pm102$ against $516\pm94$~s pooled over both splits, paired $p=0.002$; per split, $p=0.028$ on \texttt{random} and $p=0.047$ on \texttt{extrap:alpha\_max}, Table~\ref{tab:backend}), for two roughly equal reasons: JAX needs 30\,\% more time per epoch (paired $p=0.005$, $n=10$) and 29\,\% more epochs before early stopping ($p=0.021$; the 800-epoch cap was reached in two JAX runs and one PyTorch run). % src: tables/all_tables.tex tab:backend (wall time: random p = 0.028, extrap:alpha_max p = 0.047, pooled p = 0.002; s/epoch pooled p = 0.005; epochs pooled p = 0.021) % EDIT: p = 0.0009 (unpaired) → paired 0.005; per-split T4 rows cited instead of the pool alone
...
% EDIT: the dataset of this comparison is stated explicitly; numbers checked against ERRATA.md E-24.
...
\begin{table*}[t]
\caption[Accuracy and physical admissibility of the two-dimensional families]{Accuracy and physical admissibility of the three two-dimensional families on a common hold-out of 261 cases. Accuracy: component RMSE (MPa)\,/\,$R^2$ and macro-RMSE; admissibility: the dimensionless equilibrium residuals $|R_r|$, $|R_z|$ and the traction-free violation $|\sigma_{rr}(r{=}1)|$, MPa. Definitions, denominators and the training pool are given in the text.}
...
% stated in the paragraphs around the table. % EDIT: caption made descriptive; definitions moved to the text; R² denominator named — contours.py computes 1 − SS_res/SS_tot over the 261 test cases
...
% NOTE: the reference to earlier figures removed; the definition that matters is stated directly. % src: ERRATA.md E-01 (R_EXTRAP_FREE_SURFACE = 1.5; condition imposed at 1.5R, measured at r = 1), E-18 (component labels); MANUSCRIPT.md §6.2 % EDIT: sentence added
...
The ablation in Table~\ref{tab:ablation2d} isolates the axial equation; its residuals are evaluated by the training code in MPa/m on the 1756-case splits and are not numerically comparable to the dimensionless medians of Table~\ref{tab:admissibility}. % src: runs_v2.csv stage twod (eq_res_median, eq_z_full); RESULTS.md §5 % EDIT: moved from the caption
...
% NOTE: the corruption experiment is referred to the table that carries its values; no figure of it is set in the manuscript.
...
% EDIT: Limitations paragraph added at the end of the section in place of the undefined \ref{sec:limitations}; wording from MANUSCRIPT.md §3, with v2 figures in place of that draft's NRMSE values (0.58 / 2.2–2.4 are v1 and are not used).
...
\begin{figure*}[t]
    ...
    \caption[Residual stress components: finite element against the three families]{Distribution of the residual stress tensor components over the investigated region for FEM, MLP, and PINN in the full and reduced formulations at $Q = 0.025$ (2.5\,\% in diameter, 5.4\,\% in area), $k = 0.1$, $\alpha = 8^\circ$, $\mu = 0.10$, $v = 20$~m/min. Rows: FEM reference and the three model families; columns: $\sigma_{rr}$, $\sigma_{\theta\theta}$, $\sigma_{zz}$, $\tau_{rz}$. The colour scale is common to all rows within a component. $r$ is normalised by the surface radius in the same cross-section ($r/R = 1$ is the free surface), $z$ by the window length. The case shown has the median test error of the full PINN.}
...
\begin{table*}[t]
\centering
\caption[Macro-averaged RMSE on the 13 splits of the main track]{Macro-averaged RMSE on the held-out cases for the 13 splits of the main track, MPa, mean $\pm$ sd over 5 seeds. Split types are defined in Section~\ref{sec:fem-dataset}.}
\label{tab:main}
\small
...
\begin{table*}[t]
\centering
\caption[The strong-form PINN in PyTorch and in JAX]{The same strong-form PINN implemented twice, in PyTorch and in JAX, on the \texttt{random} and \texttt{extrap:alpha\_max} splits, 5 seeds each: accuracy (macro-RMSE, MPa) and cost. Mean $\pm$ sd over seeds; $p$ from a seed-paired $t$-test. Timings are for CPU, double precision.}
\label{tab:backend}
\small
...
\begin{table*}[t]
\centering
\caption[Robustness to label corruption]{Robustness to label corruption on the \texttt{random} split: macro-RMSE on the clean hold-out, MPa, mean $\pm$ sd over 3 seeds. Below the rule: the seed-paired gap to the data-driven baseline and its regression on the corrupted fraction.}
\label{tab:corrupt}
\small
\begin{tabular}{@{}lrrrrr@{}}
\toprule
...
% NOTE: an introductory paragraph is placed before the list, following the agreed outline (a series of
...
% NOTE: the list is shortened from seven items to five, and the items that rested on differences that were not
...
%   - item (7) of the compiled version is deleted here and treated in the closing paragraph (see % NOTE:below);
%   - the clause of item (3) withdrawing a conclusion of an earlier text is deleted (see % NOTE:below);
...
\item The two software backends, PyTorch and JAX, are statistically indistinguishable in accuracy and differ only in cost, PyTorch completing a full training run 1.65~times faster on CPU in double precision (Table~\ref{tab:backend}). The choice of backend is thus a matter of computational cost and not of the result obtained. % NOTE: the clause withdrawing a conclusion of an earlier text is deleted; the statement stands on its own.
...
% TODO: the quantitative outcome of that comparison (MAPE 8.8 % for Siebel and 5.7 % for Avitzur, and the agreement
...
\entry{$R$}{billet radius (Fig.~\ref{fig:fem-fields}) or surface radius of the cross-section (Figs.~\ref{fig:compare-fields}--\ref{fig:residual-maps})}
...
\entry{$R_0,R_f$}{radius of the billet and of the drawn wire (m)}
\entry{$v$}{drawing velocity (m/min)}
\entry{$p$}{normal pressure on the die contact (MPa)}
\entry{$m$}{friction factor, ratio of the interface shear stress to the shear yield stress}
\entry{$f(\alpha)$}{die-angle function of the Avitzur velocity field}
\entry{$c$}{specific heat (J\,kg$^{-1}$K$^{-1}$)}
\entry{$T$}{temperature ($^\circ$C)}
\EntryHeading{Greek Letters}
\entry{$\alpha$}{half-angle of the die working cone (deg)}
\entry{$\mu$}{Coulomb friction coefficient}
\entry{$\sigma_d$}{drawing stress (MPa)}
\entry{$\sigma_f$}{flow stress (MPa)}
\entry{$\varepsilon_{\mathrm{hom}},\varepsilon_{\mathrm{eff}}$}{homogeneous and effective strain}
\entry{$\dot\varepsilon$}{mean strain rate over the pass (s$^{-1}$)}
\entry{$\Delta$}{inhomogeneity parameter of the deformation zone}
\entry{$\Phi$}{excess-work factor}
\entry{$\beta$}{fraction of plastic work converted to heat}
\entry{$\rho$}{density (kg\,m$^{-3}$)}
