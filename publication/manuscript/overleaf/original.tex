...
initial temperature of $20^\circ$C throughout.
...
by averaging the constitutive equation along the deformation path of each material point,
weighted by plastic work, and then over the section weighted by radius:
...
The two solutions used are, in the slab-method form of Siebel,
\begin{equation}
...
against direct numerical integration to eight digits and reducing to $\ln[1/(1-r)]$ as
$\alpha\to0$), $m$ is the friction factor and $L$ the land length.
% TODO: keys TODO-siebel, TODO-avitzur to be added to the bibliography for Eqs. (\ref{eq:siebel}), (\ref{eq:avitzur}).
...
\begin{table}[htbp]
\centering\small
\caption{Drawing stress $\sigma_d$ (MPa): the finite-element value against the Siebel,
Eq.~(\ref{eq:siebel}), and Avitzur, Eq.~(\ref{eq:avitzur}), solutions, evaluated with the flow
stress derived from the constitutive equation and no fitted parameter. Reduction 1.6\,\%, land
ratio unity. The Avitzur column awaits the revised friction term; with the provisional factor
$m=\sqrt3\,\mu\,p/\sigma_f$ at the measured contact pressure $p=122$~MPa it gives 65.1, 71.9, 85.9
and 69.5~MPa.}
\label{tab:drawstress}
\begin{tabular}{@{}llrrr@{}}
\toprule
$\alpha$ & $\mu$ & FEM & Siebel & Avitzur \\
\midrule
$12^\circ$ & 0.025 & 54.1 & 53.0 & --- \\
$12^\circ$ & 0.050 & 62.0 & 54.3 & --- \\
$12^\circ$ & 0.100 & 78.3 & 57.5 & --- \\
$8^\circ$  & 0.100 & 62.3 & 44.9 & --- \\
\midrule
\multicolumn{3}{@{}l}{mean absolute deviation} & 17.3\,\% & 13.0\,\% \\
\bottomrule
\end{tabular}
\end{table}
% src: publication/reexport/flow_stress.py (FEM sigma_d, sigma_f); publication/code/analytical.py
% siebel(r,alpha,mu,sigma_f) -> 53.0/54.3/57.5/44.9 MPa, MAPE 17.3 %; Avitzur MAPE 13.0 % from the
% section prose (parallel session) — the revised friction term replaces the "---" column.

The comparison can be closed completely, with no quantity taken from the finite-element model. The
homogeneous strain is $2\ln(R_0/R_f)$ and the total strain follows from Backofen's excess-work
...
\end{align}
where $\Delta$ is the inhomogeneity parameter and $\langle\sigma_f\rangle$ the plastic-work-weighted
mean of the strain term of the constitutive equation, taken before its rate and temperature
multipliers. The temperature
does not need a separate source, because it closes on the drawing stress itself: the drawing work
per unit volume equals $\sigma_d$, a fraction $\beta$ of which becomes heat, so the mean rise is
$\Delta T = \beta\,\sigma_d/(\rho c)$; since $\sigma_d$ depends on the flow stress and the flow
stress on the temperature, this is a fixed point, reached in five iterations. For the four runs it
returns a flow stress of 330--338~MPa, consistent with the value derived from the constitutive
equation (Section~\ref{sec:prep-data}), and a drawing stress that departs from the finite-element
value by 14.3\,\% on average, again over-predicting at $\mu = 0.025$ and under-predicting at
$\mu = 0.1$; the friction term, not the flow stress, is the source of the discrepancy.
% src: publication/reexport/analytic_sigma_f.py (--Q 0.0164 --v 40; alpha 12/8, mu 0.025/0.05/0.1):
% eps_hom = 0.033, Phi = 6.5, eps_eff = 0.215 (alpha 12) / 0.153 (alpha 8), edot = 15.6 / 10.3 s^-1,
% sigma_f = 338.2/338.1/337.9/330.1 MPa; the self-consistent temperature is used below.

\begin{table*}[htbp]
\centering\small
\caption{Intermediate quantities of the closed self-consistent estimate, Eqs.~(\ref{eq:closed-strain})--(\ref{eq:closed-rest}),
...
the constitutive equation at the fixed-point temperature, and the resulting drawing stress
$\sigma_d$. Nothing is taken from the finite-element model; $\varepsilon_{\mathrm{hom}}=0.033$ and
$R_f=17.705$~mm throughout.}
\label{tab:closed}
\begin{tabular}{@{}llrrrrrr@{}}
\toprule
$\alpha$ & $\mu$ & $\Delta$ & $\Phi$ & $\varepsilon_{\mathrm{eff}}$ & $\dot\varepsilon$, s$^{-1}$ & $\sigma_f$, MPa & $\sigma_d$, MPa \\
\midrule
$12^\circ$ & 0.025 & 25.1 & 6.52 & 0.215 & 15.6 & 338.2 & 67.4 \\
$12^\circ$ & 0.050 & 25.1 & 6.52 & 0.215 & 15.6 & 338.1 & 68.7 \\
$12^\circ$ & 0.100 & 25.1 & 6.52 & 0.215 & 15.6 & 337.9 & 71.2 \\
$8^\circ$  & 0.100 & 16.8 & 4.63 & 0.153 & 10.3 & 330.1 & 54.3 \\
\bottomrule
\end{tabular}
\end{table*}
% src: publication/reexport/analytic_sigma_f.py --Q 0.0164 --v 40 (alpha 12/8, mu 0.025/0.05/0.1);
% sigma_d 67.4/68.7/71.2/54.3 MPa against FEM 54.1/62.0/78.3/62.3 (mean absolute deviation 14.3 %).
...
measured in the model to within 11\,\% (Table~\ref{tab:deltaT}); the heating can therefore be taken
from the drawing work itself, with this comparison as its justification, rather than from an assumed
thermal boundary condition.

\begin{table}[htbp]
\centering\small
\caption{Mean temperature rise during the pass: the self-consistent analytical estimate, in which
the drawing stress and the temperature close on each other and nothing is read from the model,
against the cross-section average of the exported temperature history. Reduction 1.6\,\%, land
ratio unity.}
\label{tab:deltaT}
\begin{tabular}{@{}llrr@{}}
\toprule
$\alpha$ & $\mu$ & $\Delta T$ analytical, K & $\Delta T$ measured, K \\
\midrule
$12^\circ$ & 0.025 & 16.4 & 14.7 \\
$12^\circ$ & 0.050 & 16.7 & 16.3 \\
$12^\circ$ & 0.100 & 17.3 & 20.2 \\
$8^\circ$  & 0.100 & 13.2 & 15.3 \\
\bottomrule
\end{tabular}
\end{table}
% src: publication/reexport/analytic_sigma_f.py (analytical); publication/reexport/read_history.py
% (measured, cross-section-averaged rise over the steady window).
...
\begin{table*}[t]
\caption{Cost of holding out a whole factor level, averaged over the three families: macro-RMSE on the held-out region, on the matched control of the same size, their ratio, $R^2_{\mathrm{n}}$ (global denominator) on the held-out region, and the constant predictor (MPa).} % EDIT: denominator named
\label{tab:axis}
\centering\small
