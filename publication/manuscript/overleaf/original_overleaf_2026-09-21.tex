...
\begin{equation}
\sigma_d = (A+B\varepsilon^n)(1+C\log(\frac{\dot{\varepsilon}}{\dot{\varepsilon_{0}}}))(1-(\frac{T-T_{room}}{T_{melt}-T_{room}})^m)\label{eq3}
\end{equation}
...
The FEM  models  were verified by comparing them with the well-known analytical expressions for tensile stresses derived by Zibel and Avitzur, and their modifications ().

In order to achieve this objective, frames corresponding to a steady-state drawing process—where the wire is in contact with the die—were selected from the calculations obtained, and the average drawing forces were determined. The calculation of drawing stress is performed using the following formula:

\begin{equation}
\sigma =\frac{F}{S}  
\end{equation}

In order to evaluate drawing stresses through the utilisation of analytical equations, it is also necessary to determine the yield strength. As indicated by the equation (), the process necessitates the provision of values for strain, strain rate, and temperature. The strain experienced during the drawing process can be expressed by the following equation:

\begin{equation}
\varepsilon =ln\frac{A_{1}}{A_{0}}  
\end{equation}

The ratio for the strain rate is composed of the following elements:

\begin{equation}
\dot\varepsilon = \frac{6\,v\,R_f^2\tan\alpha\,\ln(R_0/R_f)}{(R_0^3-R_f^3)}  
\end{equation}
The temperature resulting from the drawing process is estimated as follows:
\begin{equation}
\Delta T = \frac{\beta \sigma_d}{\rho c}
\end{equation}

\begin{equation}
T = \frac{T_{0} + \Delta T}{2}
\end{equation}
...
\hline
\multicolumn{5}{c}{Parameters} & \multicolumn{3}{c}{Analytical} & Simulation \\ \hline
$Q$ & $\alpha$ & v & $k$ & $\mu$ & Zibel & Avitzur & Avitzur_{mod} & FEM \\ \hline
 & & & & & & & & \\ \hline
 & & & & & & & & \\ \hline
\end{tabular}
\end{table}
...
%Verification of the simulation results was carried out by comparison with the established analytical solutions for the drawing stress, those of Siebel and of Avitzur, and by internal consistency tests of the exported fields.
...
%The drawing stress cannot be read from the cases used for the surrogates, whose exported frame is taken after the wire has left the die and the section is unloaded (median $|\int\sigma_{zz}\,\mathrm{d}A|/A$ of 5.3~MPa against an rms $\sigma_{zz}$ of 167~MPa).
% src: VALIDATION.md §1 (5.3 / 167 MPa on the 1756 retained cases)
%It is obtained instead from a dedicated export, in which the reaction at the die reference point, the temperature and the equivalent plastic strain are written as histories in time for four runs at a reduction of 1.6\,\% and a land ratio of unity. The axial reaction is averaged over the steady phase of the pass, defined by a die travel of 20 to 100~mm; the signal there is not a plateau but a stick--slip sawtooth whose relative spread is 13.6\,\% at $\mu = 0.025$ and 4.5\,\% at $\mu = 0.1$, so the time average is used, the quantity an energy method estimates. Divided by the area actually reached in the model, 985~mm$^2$, this gives a drawing stress of 54.1, 62.0 and 78.3~MPa at $\alpha = 12^\circ$ for $\mu = 0.025$, 0.05 and 0.1, and 62.3~MPa at $\alpha = 8^\circ$.
...
%Evaluated with the representative flow stress derived from the constitutive equation (Section~\ref{sec:prep-data}), 300--305~MPa for these runs, and with no fitted parameter, the Siebel solution deviates from the finite-element drawing stress by 17.3\,\% on average and the Avitzur upper-bound solution by 13.0\,\%. The two cannot be ranked on four points: the difference between them is smaller than the 260--324~MPa spread of the flow stress across the section. More informative than the magnitude is the sign, which changes with friction: at $\mu = 0.025$ the Avitzur solution is 11\,\% above the computed value and at $\mu = 0.1$ it is 18\,\% below. The discrepancy therefore lies in the form of the friction term rather than in the calibration of the flow stress, which enters as a multiplier and shifts every prediction together.
% src: publication/reexport/flow_stress.py; lens_analyt/analytical.py (siebel, avitzur)

%The comparison can be closed completely, with no quantity taken from the finite-element model. The homogeneous strain is $2\ln(R_0/R_f)$ and the total strain follows from Backofen's excess-work factor $\Phi = 0.8 + \Delta/4.4$; the strain rate is obtained from the time to traverse the cone at constant volume flow,

%; and the flow stress is the work-weighted mean of the constitutive equation over that path. The temperature
%does not need a separate source, because it closes on the drawing stress itself: the drawing work per unit volume equals $\sigma_d$, a fraction $\beta$ of which becomes heat, so the mean rise is


since $\sigma_d$ depends on the flow stress and the flow
...
%The temperature closure can be checked directly, since the dedicated export also carries the temperature history. The self-consistent estimate reproduces the cross-section-averaged rise measured in the model to within 11\,\% (Table~\ref{tab:deltaT}); the heating can therefore be taken from the drawing work itself, with this comparison as its justification, rather than from an assumed thermal boundary condition.
...
%Three properties of the exported histories support the extraction independently of the analytical comparison. The friction coefficient is recovered from the data rather than read from the file name: once the cone has left the wire and only the land remains in contact, the ratio of the axial to the radial reaction returns 0.02500, 0.05000 and 0.10000. The mean contact pressure is 122~MPa and is insensitive to both the semi-angle and the friction, as it must be when a cylindrical land
%dominates the contact. And the flow stress recovered from the heat alone, over the core of the section where friction contributes none, is 306--320~MPa, in agreement with the value derived from the constitutive equation.

%The region of the design covered by these runs limits what the comparison establishes. All four lie at a reduction of 1.6\,\%, where the inhomogeneity parameter is 25.5 at $\alpha = 12^\circ$ and
%17.1 at $\alpha = 8^\circ$, against the range of 1 to 3 for which upper-bound solutions were derived; the ideal work accounts for 13 to 19\,\% of the answer and the total work exceeds it by a factor of 5.3 to 7.5. The agreement to within 13 to 17\,\% therefore characterises the accuracy of the closed-form solutions in this corner, not the accuracy of the finite-element model, and does not transfer to the industrial range of reductions.
