...
the billet radius is denoted $R_0$; % REV К3: the numerical value of $R_0$ and the length of the billet are not repeated here; they belong to the description of the geometry. % REV К3 (critic): the claim that those values are "stated in the preceding subsection" was removed — Section 2.1 as it stands gives the notation $D_0$, $D$, $L$ but no numerical values, so the cross-reference asserted something the manuscript does not yet contain. % TODO К3: the supervisor undertook to give the billet radius and the billet length with the geometry in Section 2.1; if they do not appear there, the value has to be reinstated at this point. % src: ERRATA.md E-22 (D0 = 36 mm from the die part name DIE_..._D0_36); code/raw_probe.py billet_radius
