...
the billet radius is denoted $R_0$; % NOTE: the numerical value of $R_0$ and the length of the billet are not repeated here; they belong to the description of the geometry. % NOTE: the claim that those values are "stated in the preceding subsection" was removed — Section 2.1 as it stands gives the notation $D_0$, $D$, $L$ but no numerical values, so the cross-reference asserted something the manuscript does not yet contain. % TODO: the review undertook to give the billet radius and the billet length with the geometry in Section 2.1; if they do not appear there, the value has to be reinstated at this point. % src: ERRATA.md E-22 (D0 = 36 mm from the die part name DIE_..._D0_36); code/raw_probe.py billet_radius
...
the volumetric strain is purely elastic ($K = 151.0$~GPa, i.e.\ $\nu = 0.268$ at $E = 210$~GPa; fit on the $v = 250$~m/min batch, see below), and the temperature field was not among the exported quantities, so its magnitude is not established directly; the estimate of the adiabatic heating below places it below the resolution of the exported fields. % src: VALIDATION.md §3 (item 3: fit for the v = 250 batch, ΔT_eq = 0.0 K)
...
% EDIT: verification paragraph added in place of the undefined Section~\ref{sec:verification}: five internal consistency tests on the 1756 retained cases, VALIDATION.md §7.
% NOTE:/К18 (сквозное правило 4): the opening of the verification paragraph no longer sets the internal tests against a missing experiment; it opens with the comparison of the simulation results against the classical analytical relations for the drawing stress.
Verification of the results of the simulation rests on two independent checks. The first is a comparison with the classical analytical descriptions of the process, the slab-method relation of Siebel~\cite{TODO-siebel} and the upper-bound solution of Avitzur~\cite{TODO-avitzur}, which give the drawing stress as a function of the reduction, the die semi-angle and the friction coefficient. Such a comparison can be carried out only on runs of the same finite-element model whose exported frame is captured while the wire is still engaged in the die, because in the retained cases the cross-section is unloaded and carries no drawing load. % src: VALIDATION.md §4 (Siebel and Avitzur implemented, six independent (Q, α) cells, no fitted parameters), §5 item 1 (the comparison is made on a subset of runs of the same model, disjoint from the 1756 cases) % TODO: the quantitative outcome of that comparison is not yet written into the manuscript, and the two relations have no bibliography entries yet (TODO-siebel, TODO-avitzur).
...
% NOTE: the framing "one consequence has to be stated explicitly" was dropped — the comment asks for the point to be made less, not more, emphatically; and the statement is now confined to the retained cases, so that it no longer contradicts the verification paragraph above, where the drawing stress is obtained from the runs whose frame is captured inside the die.
...
Coordinates in the field plots are dimensionless throughout. In Fig.~\ref{fig:fem-fields} the radius is normalised by the billet radius, written $R$ in the axis label of that figure, so that the drawn part of the wire ends at $r/R = 0.9$, and the axial coordinate by the length of the model; % NOTE: the numerical value of the billet radius is not repeated in the text; it is retained in the caption of the figure. % NOTE: with the value gone, $R$ was left undefined here and clashed with $R$ = surface radius used elsewhere in this subsection; the sentence now says which radius the axis label of Fig.~\ref{fig:fem-fields} denotes. % src: captions.md Fig. 2 (R = 18 mm; L = 139.5 mm; drawn part ends at r/R = 0.9 at Q = 0.10)
