...
% \ref{sec:models} in sec23_models_protocol.tex, \ref{sec:results}, \ref{sec:res_extrap}, \ref{sec:res_backend},
% \ref{sec:res_fem}, \ref{tab:axis}, \ref{tab:ablation2d}, \ref{tab:admissibility} in sec32_results.tex, and
% \ref{tab:main}, \ref{tab:paired}, \ref{tab:curves}, \ref{tab:backend}, \ref{tab:corrupt} in tables/all_tables.tex.
...
% src: tables/all_tables.tex tab:paired (Holm correction over the thirteen splits: smallest adjusted p = 0.079 for PINN−MLP, 0.195 for VPINN−MLP);
...
\item The contribution of the physics term lies in the admissibility of the predicted field and in the resistance to defects of the data source, not in accuracy. At equal accuracy the strong-form and full-form networks reduce the radial and axial equilibrium residuals and the violation of the traction-free condition well below those of the data-driven baseline, and, for the boundary condition, below the finite-element source itself (Table~\ref{tab:admissibility}); the improvement in axial equilibrium is traceable to the axial term of the loss rather than to the physics term in general (Table~\ref{tab:ablation2d}). As the training labels are corrupted, the gap in favour of the physics-informed model opens monotonically, by 14.2~MPa per unit corrupted fraction ($p=0.0004$), whereas on clean labels the two are indistinguishable (Table~\ref{tab:corrupt}).
...
% src: Table~\ref{tab:admissibility} (τ_rz: R² = 0.934–0.935 with the held-out slice as denominator, two-dimensional families).
