...
Замечание руководителя (13.09.2026): предел текучести нельзя брать как одно
измеренное число, он следует из уравнения состояния и определён при конкретных
деформации, скорости деформации и температуре.
...
Карта материала (из .inp, прислана руководителем 12.09.2026):
    *Plastic, hardening=JOHNSON COOK   A = 213 МПа, B = 53 МПа, n = 0.345,
                                       m = 0.81, T_melt = 1623.15, T_transition = 293.15
...
ровно 20.0 во всех узлах), а температуры перехода и плавления записаны
кельвиновскими значениями. Поэтому приведённая температура тождественно равна
нулю на всём диапазоне расчёта (20...95), и термический множитель равен единице:
термическое разупрочнение в этих расчётах не работает по построению.
...
M_JC, T_MELT, T_TRANS = 0.81, 1623.15, 293.15
