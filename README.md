...
- Индексы плоскостей Abaqus не совпадают с физическими компонентами тензора. Перестановки **разные** для напряжений и деформаций — `PLANE_ORDER_STRESS = [2, 0, 1, 3]`, `PLANE_ORDER_STRAIN = [1, 0, 2, 3]` (детали и обоснование см. `docs/data_format.md`). При сравнении прогноза с эталоном перестановка применяется к обоим массивам — иначе R² нормальных компонент уходит в отрицательную область.
