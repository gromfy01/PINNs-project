...
> **Пересабмит статьи.** Материалы работы над ошибками по отклонённой рукописи
> — в каталоге [`publication/`](publication/): план работ
> ([`PLAYBOOK.md`](publication/PLAYBOOK.md)), реестр ошибок
> ([`ERRATA.md`](publication/ERRATA.md)) и аудит сырых данных
> ([`RAW_AUDIT.md`](publication/audit/RAW_AUDIT.md)). Часть утверждений в
> `docs/` и в моделях там уточнена по результатам аудита — начинать чтение
> лучше оттуда.
...
├── publication/       # Пересабмит статьи: план работ, реестр ошибок, аудит данных
...
- При сравнении прогноза с эталоном применяется перестановка компонент `PLANE_ORDER = [2, 0, 1, 3]` к **обоим** массивам — этой перестановкой пользуются и stress-, и strain-пайплайны для своих обучающих pkl. Детали см. в `docs/data_format.md`.
